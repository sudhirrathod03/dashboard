...
const Orders = () => {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    // Since we only use stable state setters, the dependency array is empty.
  }, []);

  // ✅ Listen for order refresh trigger
  useEffect(() => {
    // This effect now correctly depends on the trigger, whether orders are visible,
    // and the fetch function itself.
    if (showOrders && generalContext.orderRefreshTrigger > 0) {
      fetchOrders();
    }
  }, [generalContext.orderRefreshTrigger, showOrders, fetchOrders]); // ✅ Fix: Add missing dependencies

  return (
    <div className="orders">
      {!showOrders ? (
        <div className="no-orders">
          {/* We now call the memoized fetchOrders function here */}
          <button className="btn" onClick={fetchOrders} disabled={loading}>
            {loading ? "Loading..." : "Get started"}
          </button>
          {error && <p className="error">{error}</p>}
