...
const Holdings = () => {
  ...
  useEffect(() => {
    axios
      ...
      .catch((err) => {
        ...
        setAllHoldings([]); // Ensure it stays an array even on error
        setLoading(false);
      });
  }, []);

  // CRITICAL FIX: Add safety check before using allHoldings
  if (loading) {
    return <div className="loading">Loading holdings...</div>;
  }
  ...
  // Safety check - ensure allHoldings is an array
  if (!Array.isArray(allHoldings)) {
    console.error("allHoldings is not an array:", allHoldings);
    return <div className="error">Data format error. Please refresh.</div>;
  }
